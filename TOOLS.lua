...
-- ─── BILIS NG TAKBO  (+6 / −6 value card) ────────────────────────
divider(body1, 39)
sectionHead(body1, "Run Speed", 40)
local bilisVC = makeValueCard(body1, "BILIS NG TAKBO", runSpeed, 41)
...
-- ══════════════════════════════════════════════════════════════════
--  BILIS NG TAKBO  (+6 / −6 per click)
-- ══════════════════════════════════════════════════════════════════
bilisVC.plusBtn.MouseButton1Click:Connect(function()
	runSpeed = math.min(runSpeed + 6, 1000)
	BASE_SPRINT_SPEED = runSpeed
	bilisVC.val.Text = tostring(runSpeed)
	updateCharacterReferences()
	if walkEnabled and humanoid then humanoid.WalkSpeed = runSpeed end
end)
bilisVC.minusBtn.MouseButton1Click:Connect(function()
	runSpeed = math.max(runSpeed - 6, 2)
	BASE_SPRINT_SPEED = runSpeed
	bilisVC.val.Text = tostring(runSpeed)
	updateCharacterReferences()
	if walkEnabled and humanoid then humanoid.WalkSpeed = runSpeed end
