...
local spinAngle          = 0
local spinSpeed          = 6   -- degrees per frame default (Slow)
...
local tpLoopEnabled      = false   -- new bonus feature state
...
wpCard.Size             = UDim2.new(1, 0, 0, 80)
...
-- Row 1: Slow · Normal · Fast · Faster  (4 buttons)
local presetRow1 = Instance.new("Frame")
presetRow1.Size                   = UDim2.new(1, -18, 0, 24)
presetRow1.Position               = UDim2.new(0, 9, 0, 22)
...
-- Row 2: Mabilis · Motor · Kabayo  (3 buttons)
local presetRow2 = Instance.new("Frame")
presetRow2.Size                   = UDim2.new(1, -18, 0, 24)
presetRow2.Position               = UDim2.new(0, 9, 0, 50)
...
local ROW1_COUNT = 4   -- indices 1-4 go to row 1
-- row1: 4 btns, gap 3×4=12, width = (280-12)/4 = 67
-- row2: 3 btns, gap 2×4=8,  width = (280-8)/3  = 90.6 → 90
...
-- ─── ADVANCED (locally sovereign — no server needed) ──────────────
divider(body1, 72)
...
-- ══════════════════════════════════════════════════════════════════
--  FULLBRIGHT  — rewrites local Lighting, server perceives nothing
-- ══════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════
--  LOW GRAVITY  — workspace.Gravity is a local property, pure client
-- ══════════════════════════════════════════════════════════════════
local function setLowGrav(state)
	...
	if state then
		...
		workspace.Gravity    = 20   -- default is 196.2; 20 = moon-tier floatiness
	else
...
-- ══════════════════════════════════════════════════════════════════
--  SPIN BOT  — 3 speeds, only one active at a time
--  Slow = 4°/frame · Fast = 14°/frame · VERY FAST = 36°/frame
-- ══════════════════════════════════════════════════════════════════
...
spinSlowToggle.button.MouseButton1Click:Connect(function()
	if spinEnabled and spinSpeed == SPIN_SPEEDS.slow then stopSpin()
	else startSpin(SPIN_SPEEDS.slow, spinSlowToggle, spinFastToggle, spinRageToggle) end
end)
spinFastToggle.button.MouseButton1Click:Connect(function()
	if spinEnabled and spinSpeed == SPIN_SPEEDS.fast then stopSpin()
	else startSpin(SPIN_SPEEDS.fast, spinSlowToggle, spinFastToggle, spinRageToggle) end
end)
spinRageToggle.button.MouseButton1Click:Connect(function()
	if spinEnabled and spinSpeed == SPIN_SPEEDS.rage then stopSpin()
	else startSpin(SPIN_SPEEDS.rage, spinSlowToggle, spinFastToggle, spinRageToggle) end
end)

-- ══════════════════════════════════════════════════════════════════
--  FREEZE SELF  — anchors your HumanoidRootPart locally
--  Purely a client-side BasePart.Anchored write
-- ══════════════════════════════════════════════════════════════════
...
-- auto-unfreeze on respawn so player isn't stuck forever
...
-- ══════════════════════════════════════════════════════════════════
--  ZOOM HACK  — extends local camera MaxZoomDistance
--  CameraMaxZoomDistance is a StarterPlayer/LocalPlayer property
-- ══════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════
--  TP LOOP  — teleports you to a random player every 3 seconds
--  Pure CFrame write on your own rootPart — 100% local
-- ══════════════════════════════════════════════════════════════════
