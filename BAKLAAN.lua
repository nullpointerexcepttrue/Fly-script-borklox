...
local fakeAfkEnabled     = false
local fakeAfkBillboard   = nil
...
local spinToggle        = makeToggleRow(body1, "Spin Bot",      79)
local freezeToggle      = makeToggleRow(body1, "Freeze Self",   80)
local zoomToggle        = makeToggleRow(body1, "Zoom Hack",     81)
local fakeAfkToggle     = makeToggleRow(body1, "Fake AFK Tag",  82)
...
-- ══════════════════════════════════════════════════════════════════
--  SPIN BOT  — rotates HumanoidRootPart CFrame each RenderStepped
--  Server sees the character move but spin rate is client-authored
-- ══════════════════════════════════════════════════════════════════
local SPIN_RATE = 8   -- degrees per frame at 60fps

local function setSpin(state)
	spinEnabled = state
	spinToggle.refresh(state)
	if spinConnection then spinConnection:Disconnect(); spinConnection = nil end
	if state then
		spinConnection = RunService.RenderStepped:Connect(function(dt)
			updateCharacterReferences()
			if not rootPart or not rootPart.Parent then return end
			spinAngle = (spinAngle + SPIN_RATE) % 360
			local pos = rootPart.Position
			rootPart.CFrame = CFrame.new(pos) * CFrame.Angles(0, math.rad(spinAngle), 0)
		end)
	end
end
spinToggle.button.MouseButton1Click:Connect(function() setSpin(not spinEnabled) end)
...
player.CharacterAdded:Connect(function()
	if freezeEnabled then
		freezeEnabled = false
		task.defer(function()
			if freezeToggle then freezeToggle.refresh(false) end
		end)
	end
end)
...
-- ══════════════════════════════════════════════════════════════════
--  FAKE AFK TAG  — BillboardGui parented to your own Head
--  Replicates to other players naturally via Roblox character sync
-- ══════════════════════════════════════════════════════════════════
local function removeFakeAfkBillboard()
	if fakeAfkBillboard and fakeAfkBillboard.Parent then
		fakeAfkBillboard:Destroy()
	end
	fakeAfkBillboard = nil
end

local function createFakeAfkBillboard()
	updateCharacterReferences()
	local head = character and character:FindFirstChild("Head")
	if not head then return end

	removeFakeAfkBillboard()
	...
	bb.MaxDistance  = 60
	bb.Adornee      = head
	bb.Parent       = head   -- parent to Head = replicates to server
	...
	dot.BackgroundColor3 = Color3.fromRGB(255, 200, 40)
	...
	lbl.Position       = UDim2.new(0, 16, 0, 0)
	lbl.Text           = "💤 AFK"
	lbl.TextColor3     = Color3.fromRGB(255, 200, 40)
	lbl.TextSize       = 13
...
local function setFakeAfk(state)
	fakeAfkEnabled = state
	fakeAfkToggle.refresh(state)
	if state then
		createFakeAfkBillboard()
	else
		removeFakeAfkBillboard()
	end
end
fakeAfkToggle.button.MouseButton1Click:Connect(function() setFakeAfk(not fakeAfkEnabled) end)

-- re-create tag on respawn if still enabled
player.CharacterAdded:Connect(function(char)
	if fakeAfkEnabled then
		task.wait(0.5)
		createFakeAfkBillboard()
	end
end)
