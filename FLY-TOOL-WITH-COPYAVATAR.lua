-- ╔══════════════════════════════════════════════════════════════╗
-- ║   TRAPVERSE FLYER  ·  Harvey Specter Edition  v4  (FIXED)   ║
-- ║   LocalScript — StarterPlayerScripts                        ║
-- ║   Features: Fly · Walk · Noclip · Inf-Jump · Tags          ║
-- ║             Click-TP · Search Player · Copy Avatar          ║
-- ╚══════════════════════════════════════════════════════════════╝
...
-- ══════════════════════════════════════════════════════════════════
local C = {
	Void        = Color3.fromRGB(5,   5,   6),
	Obsidian    = Color3.fromRGB(10,  10,  12),
	Charcoal    = Color3.fromRGB(16,  16,  19),
	Slate       = Color3.fromRGB(22,  22,  27),
	SlateHover  = Color3.fromRGB(28,  28,  34),
	Gold        = Color3.fromRGB(196, 160,  90),
	GoldBright  = Color3.fromRGB(230, 196, 126),
	GoldDim     = Color3.fromRGB( 52,  40,  14),
	GoldLine    = Color3.fromRGB( 70,  54,  18),
	Ivory       = Color3.fromRGB(242, 238, 228),
	Champagne   = Color3.fromRGB(176, 168, 150),
	Ash         = Color3.fromRGB( 72,  70,  65),
	Wire        = Color3.fromRGB( 30,  30,  36),
	WireGold    = Color3.fromRGB( 80,  62,  20),
	ActiveKnob  = Color3.fromRGB(210, 175, 100),
	InactiveKnob= Color3.fromRGB( 48,  47,  44),
...
-- ══════════════════════════════════════════════════════════════════
--  ACTION BUTTON  (full-width gold button)
-- ══════════════════════════════════════════════════════════════════
local function makeActionBtn(parent, txt, order, theme)
	theme = theme or "gold"
	local bg = theme == "green" and C.GreenDim or C.GoldDim
	local fg = theme == "green" and C.GreenBright or C.GoldBright
	local brd= theme == "green" and Color3.fromRGB(24, 68, 28) or C.GoldLine
	...
	local activeHover = theme == "green" and C.GreenBright or C.GoldBright
	local activeText  = C.Void

	b.MouseEnter:Connect(function()   tw(b, { BackgroundColor3 = activeHover, TextColor3 = activeText }) end)
	...
	b.MouseButton1Up:Connect(function()   tw(b, { BackgroundColor3 = activeHover, TextColor3 = activeText }) end)
	return b
end
...
-- ─── COPY AVATAR ─────────────────────────────────────────────────
-- FIXED: Fully client-side. No server script needed.
-- Type any Roblox username → hit COPY AVATAR → done.
-- ─────────────────────────────────────────────────────────────────
divider(body1, 74)
sectionHead(body1, "Copy Avatar", 78)

local dupInputCard, dupInput = makeInputCard(body1, "Target Username", "Enter any Roblox username…", 80)
local dupApplyBtn = makeActionBtn(body1, "⧉  COPY AVATAR", 82)
local dupStatus   = makeStatusLabel(body1, 84)
...
-- Copy from search result button (bonus!)
local copyFromSearchBtn = makeActionBtn(body2, "⧉  COPY THIS PLAYER'S AVATAR", 47)
...
local function createTag(tp, tc)
	...
	bar.Position         = UDim2.new(0, 4, 0.5, -12)
...
-- ══════════════════════════════════════════════════════════════════
--  COPY AVATAR — FIXED, FULLY CLIENT-SIDE
--  Works for ANY Roblox username, no server script needed.
-- ══════════════════════════════════════════════════════════════════
local function copyAvatarByUsername(username, statusLabel, applyBtn)
	...
	statusLabel.Text       = "⏳ Looking up " .. username .. "…"
	statusLabel.TextColor3 = C.Champagne
	if applyBtn then applyBtn.Active = false end
	...
	task.spawn(function()
		...
		if not ok or not userId then
			statusLabel.Text       = "✗ User not found: " .. username
			statusLabel.TextColor3 = C.RedBright
			if applyBtn then applyBtn.Active = true end
			return
		end

		-- Step 2: fetch their HumanoidDescription
		local avatarDesc
		ok, err = pcall(function()
			avatarDesc = Players:GetHumanoidDescriptionFromUserId(userId)
		end)

		if not ok or not avatarDesc then
			statusLabel.Text       = "✗ Could not fetch avatar data."
			statusLabel.TextColor3 = C.RedBright
			if applyBtn then applyBtn.Active = true end
			return
		end

		-- Step 3: apply to our character
		updateCharacterReferences()
		if not humanoid then
			statusLabel.Text       = "✗ Your character isn't ready."
			statusLabel.TextColor3 = C.RedBright
			if applyBtn then applyBtn.Active = true end
			return
		end
		...
		ok, err = pcall(function()
			humanoid:ApplyDescription(avatarDesc)
		end)

		if applyBtn then applyBtn.Active = true end

		if ok then
			statusLabel.Text       = "✔ Copied " .. username .. "'s avatar!"
			statusLabel.TextColor3 = C.GreenBright
		else
			statusLabel.Text       = "✗ Apply failed: " .. tostring(err)
			statusLabel.TextColor3 = C.RedBright
		end
	end)
end

-- Panel 1 copy avatar button
dupApplyBtn.MouseButton1Click:Connect(function()
	local username = dupInput.Text:match("^%s*(.-)%s*$")
	copyAvatarByUsername(username, dupStatus, dupApplyBtn)
end)

-- ══════════════════════════════════════════════════════════════════
--  SEARCH PLAYER + TELEPORT + COPY FROM SEARCH
-- ══════════════════════════════════════════════════════════════════
...
srchBtn.MouseButton1Click:Connect(function()
	...
	if found then
		...
		setSearchResult("✗ Not found in server: " .. query, C.RedBright, false, false)
	end
end)
...
tpToBtn.MouseButton1Click:Connect(function()
	...
	if not tRoot then setSearchResult("✗ Cannot find player position.", C.RedBright, false, false); return end
...
-- Copy avatar button wired to the found player's username
copyFromSearchBtn.MouseButton1Click:Connect(function()
	if not foundTargetPlayer then return end
	copyAvatarByUsername(foundTargetPlayer.Name, srchStatus, copyFromSearchBtn)
end)
